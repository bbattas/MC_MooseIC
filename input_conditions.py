...
b_upper = [4000,8000,4000]#100 * np.ones(dim)                      # Upper bound [x,y,z]
b_lower = 0 * np.ones(dim)                        # Lower bound [x,y,z]
N = 30                                            # Number of particles
max_ic_its = 120                                  # Maximum number of tries to generate initial condition
xmin = [2000,4000,2000]#0#[0,0,0]#[0, 0]                                   # Lowest energy location [x,y,z], drop uses 1 coordinate
...
p = MC_Main_Point(n_steps, p, dim, b_lower, b_upper, it_perParticle, disp_max, xmin, overlapWeight,pusherTF,showGraph,pltTime,saveAnimation,aniName,aniType,aniDPI)

# # Particles fall to y = xmin
# p = MC_Main_Drop(n_steps, p, dim, b_lower, b_upper, it_perParticle, disp_max, xmin, overlapWeight, dropAxis,
